...
void main() {
  group('WeatherRepository', () {
    ...
    setUp(() {
      metaWeatherApiClient = MockWeatherApiClient();
      weatherRepository =
    ...
    group('getWeather', () {
      ...
      test('throws when getWeather fails', (){
        ...
        final location = MockLocation();
        when(() => location.woeid).thenReturn(woeid);
        when(() => metaWeatherApiClient.locationSearch(any())).thenAnswer((_)async => location);
      ...
      test('returns correct weather on success (showers)', ()async {
        ...
        when(() => metaWeatherApiClient.locationSearch(any())).thenAnswer((_)async => location);
        when(() => metaWeatherApiClient.getWeather(any())).thenAnswer((_)async => weather);
        expect(await weatherRepository.getWeather(city), Weather(
          temperature: 42.42,
          location: 'London',
